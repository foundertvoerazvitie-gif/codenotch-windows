...
pub fn broadcast(app: &AppHandle) {
    ...
    let snap = {
        ...
        let accent = accent::normalize(&cfg.accent);
        let accent_hex = accent::resolve(&accent);
        let show_welcome = !cfg.welcome_seen;
        store.snapshot(
            &cfg.lang,
...
/// Logical window size for the current edge (side = tall, top/bottom = wide).
fn notch_logical_size(horizontal: bool) -> (f64, f64) {
    if horizontal {
        (NOTCH_H, NOTCH_W)
    } else {
        (NOTCH_W, NOTCH_H)
...
/// Pins the notch to the chosen screen edge; along-edge position is remembered per edge.
pub fn place_notch(app: &AppHandle) {
    ...
    if let Ok(Some(mon)) = w.primary_monitor() {
        let (edge, along, horizontal) = {
            let st = app.state::<AppState>();
            let c = st.cfg.lock().unwrap();
            (c.edge().to_string(), c.along(), c.is_horizontal())
        };
        let (lw, lh) = notch_logical_size(horizontal);
        let ms = mon.scale_factor();
        let target = tauri::PhysicalSize::new((lw * ms).round() as u32, (lh * ms).round() as u32);
        let _ = w.set_size(target);
        let (ww, wh) = w
            .outer_size()
            .map(|s| (s.width as i32, s.height as i32))
            .unwrap_or(((lw * scale) as i32, (lh * scale) as i32));
        ...
        let (x, y) = match edge.as_str() {
            "left" => {
                let y = (my as f64 + mh as f64 * along - wh as f64 / 2.0).round() as i32;
                (mx, y.clamp(my, my + (mh - wh).max(0)))
            }
            "top" => {
                let x = (mx as f64 + mw as f64 * along - ww as f64 / 2.0).round() as i32;
                (x.clamp(mx, mx + (mw - ww).max(0)), my)
            }
            "bottom" => {
                let x = (mx as f64 + mw as f64 * along - ww as f64 / 2.0).round() as i32;
                (x.clamp(mx, mx + (mw - ww).max(0)), my + (mh - wh).max(0))
            }
            _ => {
                let y = (my as f64 + mh as f64 * along - wh as f64 / 2.0).round() as i32;
        ...
        if w.outer_size().map(|s| s.width != target.width || s.height != target.height).unwrap_or(false) {
            let _ = w.set_size(target);
            let ww = target.width as i32;
            let wh = target.height as i32;
            let (x, y) = match edge.as_str() {
                "left" => (mx, y.clamp(my, my + (mh - wh).max(0))),
                "top" => (x.clamp(mx, mx + (mw - ww).max(0)), my),
                "bottom" => (x.clamp(mx, mx + (mw - ww).max(0)), my + (mh - wh).max(0)),
                _ => (mx + mw - ww, y.clamp(my, my + (mh - wh).max(0))),
            };
            let _ = w.set_position(tauri::PhysicalPosition::new(x, y));
        }
        ...
        let _ = std::fs::write(
            log,
            format!(
                "notch placed build={BUILD}: edge={edge} along={along:.3} pos=({x},{y}) size=({ww}x{wh}) inner={:?} win_scale={scale} mon_scale={ms} monitor=({},{} {}x{})\n",
...
pub fn apply_edge(app: &AppHandle, edge: &str) {
    {
        let st = app.state::<AppState>();
        let mut c = st.cfg.lock().unwrap();
        c.set_edge(edge);
...
pub fn apply_accent(app: &AppHandle, choice: &str) {
    {
        let st = app.state::<AppState>();
        let mut c = st.cfg.lock().unwrap();
        c.accent = accent::normalize(choice);
        config::save(&c);
    }
    tray::refresh_menu(app);
    broadcast(app);
...
#[tauri::command]
fn open_telegram() -> Result<String, String> {
    community::open_telegram()
}

#[tauri::command]
fn dismiss_welcome(app: AppHandle) {
    {
        let st = app.state::<AppState>();
        let mut c = st.cfg.lock().unwrap();
        c.welcome_seen = true;
...
/// Drag along the current edge. The page calls this once after a press on the pill moves more than
...
fn drag_begin(app: AppHandle) {
    ...
    std::thread::spawn(move || {
        ...
        let horizontal = matches!(edge.as_str(), "top" | "bottom");
        let mut last = if horizontal { start_pos.x } else { start_pos.y };
        let mut moved = false;
        loop {
            if !left_button_down() {
                break;
            ...
            if let Ok(cur) = app.cursor_position() {
                if horizontal {
                    ...
                    let nx = nx.clamp(mx, mx + (mw - ww).max(0));
                    if nx != last {
                        last = nx;
                        moved = true;
                        let _ = w.set_position(tauri::PhysicalPosition::new(nx, start_pos.y));
                    }
                } else {
                    let ny = (start_pos.y as f64 + (cur.y - start_cur.y)).round() as i32;
                    let ny = ny.clamp(my, my + (mh - wh).max(0));
                    if ny != last {
                        last = ny;
                        moved = true;
        ...
        if moved {
            let ratio = if horizontal {
                ((last + ww / 2 - mx) as f64 / mw as f64).clamp(0.0, 1.0)
            } else {
                ((last + wh / 2 - my) as f64 / mh as f64).clamp(0.0, 1.0)
            };
            let st = app.state::<AppState>();
            let mut c = st.cfg.lock().unwrap();
            c.set_along(ratio);
            config::save(&c);
            applog(&format!("notch drag: edge={edge} along={ratio:.3}"));
        }
        DRAGGING.store(false, std::sync::atomic::Ordering::SeqCst);
...
pub fn toggle_provider(app: &AppHandle, id: &str) {
    {
        let st = app.state::<AppState>();
        let mut c = st.cfg.lock().unwrap();
        let on = !c.providers.get(id);
        c.providers.set(id, on);
        config::save(&c);
    }
    tray::refresh_menu(app);
    broadcast(app);
