...
//!                                         used, limit, breakdown },
//!                                 onDemand: { enabled, used, limit } } }
//!      Plan & Usage has two bars. The ring follows **Auto / Cursor Models**
//!      (`autoPercentUsed` — Composer, Grok, included models). API (`apiPercentUsed`)
//!      is a separate bucket ("Other Models"). `totalPercentUsed` is a blend and
//!      must not be shown as its own row (upstream CursorUsage.swift).
//!      On the free plan used/limit are often 0 while real usage sits in
//!      breakdown.bonus — never derive a percentage from used/limit alone.
//!
...
pub fn parse_summary(v: &serde_json::Value) -> (Vec<LimitWindow>, String) {
    ...
    let mut out = Vec::new();
    // Auto / Cursor Models (Composer, Grok, …) — same bar as the dashboard; 0 is a real reading.
    // Never use totalPercentUsed here: it blends Auto + API and disagrees with Plan & Usage.
    ...
    // Enterprise / team: no plan percentages — hard overall ceiling
    if out.is_empty() {
        if let Some(overall) = usage.get("overall") {
            let enabled = overall.get("enabled").and_then(|x| x.as_bool()).unwrap_or(false);
            let limit = overall.get("limit").and_then(|x| x.as_f64()).unwrap_or(0.0);
            let used = overall.get("used").and_then(|x| x.as_f64());
            if enabled && limit > 0.0 {
                if let Some(u) = used {
                    out.push(LimitWindow {
                        id: "included".into(),
...
fn broadcast(app: &AppHandle, snap: UsageSnapshot) {
    ...
    *st.cursor.lock().unwrap() = snap.clone();
    ...
    crate::threshold::observe(app, "cursor", "Cursor", &snap);
}
